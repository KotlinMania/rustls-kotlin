# Claude Code Project Instructions — rustls-kotlin

## Project Overview

This is **rustls-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`rustls`](https://crates.io/crates/rustls).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/rustls/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/rustls>
